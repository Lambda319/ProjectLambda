...
function formatInputForParticipantLeft(request, zoom_event_id) {
    return {
        headers: {
...
async function callParticipantLeft(request) {
    const params = {
        FunctionName: "ParticipantLeft",
        InvocationType: "Event",
        Payload: JSON.stringify(request),
    };


    log("callParticipantLeft", "After params", `params: ${JSON.stringify(params)}`);

    try {
        let response = await lambda.invoke(params).promise();
        log("callParticipantLeft", "success response", `called lambda successfully : ${JSON.stringify(response)}`);
    } catch (err) {
        return buildResponse(1, 500,
            warn("callParticipantLeft", "err response", `error calling ParticipantLeft: ${JSON.stringify(err.stack)}`));
    }
}
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        } else if (request.event !== 'meeting.participant_left') {
            return buildResponse(4, 400,
                warn("main", "checking event.body.event", `Invalid event received, expected 'meeting.participant_left', request: ${JSON.stringify(request)}`));
        }
        ...
        useremail = `WH-PL-EM:${request.payload.object.participant.email || "unknown"}`;
        ...
        const participantLeftRequest = formatInputForParticipantLeft(parsedRequest, zoom_event_id);

        await callParticipantLeft(participantLeftRequest);

        return buildResponse(0, 200,
            log("main", "response", `done: ${JSON.stringify(participantLeftRequest)}`),
            participantLeftRequest.body,
            participantLeftRequest.headers);
    } catch (err) {
        return buildResponse(1, 500,
            warn("main", "err",
...
function getLogLine(funcname, process, log) {
    return `[[ Webhook-ParticipantLeft - ${funcname} ]] -> ${process}, user:${useremail}, id: ${requestID}, eventID: ${zoom_event_id}
        logged -- ${log}`;
}
