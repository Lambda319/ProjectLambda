...
function formatInputForUpdateMeetingStatus(request) {
    return {
        ...
        body: {
            ...
            status: 1, // meeting is in progress
    ...
    try {
        ...
        do {
            ...
            if (getMeetingResponse.error !== 0) { // User DNE
                if (getMeetingResponse.error !== 1 || getMeetingResponse.statusCode !== 404) { // some other error
...
function getLogLine(funcname, process, log) {
    return `[[ MeetingStarted - ${funcname} ]] -> ${process}, user:${useremail}, id: ${requestID}, logged -- ${log}`;
}
