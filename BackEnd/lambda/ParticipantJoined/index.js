...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        log("main", "getUserLocationRequest", `Request: ${JSON.stringify(getUserStatusRequest)}`);
        ...
        if (shouldUpdateUserLocation) {
            ...
            parsedRequest.topic = getMeetingResponse.body.Topic;
            parsedRequest.name = getUserStatusResponse.body.name;
        ...
        return buildResponse(1, 500,
            warn("main", "err",
                `Error in ParticipantJoined + ${JSON.stringify(err.stack) || "unknown error"}`));
    }
};
...
function getLogLine(funcname, process, log) {
    return `[[ ParticipantJoined - ${funcname} ]] -> ${process}, user:${useremail}, id: ${requestID}, logged -- ${log}`;
}
