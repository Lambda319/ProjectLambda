...
function formatInputForRDS(request) {
    return {
        ...
        functionName: "ADD_USERS_TO_GROUP",
        body: {
...
function formatInputForGetGroup(request) {
    return {
...
async function callGetGroupLambda(request) {
    ...
    try {
        ...
        log("callGetGroupLambda", "success response", `retrieved users from group: ${JSON.stringify(response)}`);
...
exports.handler = async (event, context) => {
    try {
        ...
        let addUsersToGroupRequest = formatInputForRDS(params);
        log("main", "addUsersToGroupRequest", `Request: ${JSON.stringify(addUsersToGroupRequest)}`);
        let addUsersToGroupResponse = await queryRDS(addUsersToGroupRequest);
        if (addUsersToGroupResponse.error !== 0) {
            return addUsersToGroupResponse;
        }
        log("main", "addUsersToGroupResponse", `Response: ${JSON.stringify(addUsersToGroupResponse)}`);

        //Get new group for specified user
        ...
        if (getGroupResponse.error !== 0) {
            return buildResponse(getGroupResponse.error, getGroupResponse.statusCode,
                warn("main", "GetGroupResponse error", "Users inserted, but error querying group: " + getGroupResponse.msg),
                {},
                getGroupResponse.headers);
        }

        return buildResponse(0, 200,
            log("main", "response", `Users inserted successfully, new group: ${JSON.stringify(getGroupResponse.body)}`),
        ...
        return buildResponse(1, 500,
            warn("main", "err",
                `Error in Add Users To Group " + ${JSON.stringify(err.stack) || "unknown error"}`));
    }
};
...
function getLogLine(funcname, process, log) {
    return `[[ AddUsersToGroup - ${funcname} ]] -> ${process}, user:${useremail}, id: ${requestID}
        logged -- ${log}`;
}
