...
function validateMeetingObject(event) {
    ...
    if (!event.body.meeting.recordings) {
        return buildResponse(4, 400,
            warn("main", "event.body.meeting.recordings", `Missing meeting recordings, query: ${JSON.stringify(event)}`));
    } else if (!Array.isArray(event.body.meeting.recordings)) {
        return buildResponse(4, 400,
            warn("main", "event.body.meeting.recordings", `meeting recordings should be an array, query: ${JSON.stringify(event)}`));
    }

    for (let recording of event.body.meeting.recordings) {
        if (!recording.start_time) {
            return buildResponse(4, 400,
                warn("main", "event.body.meeting.recordings.recording.start_time", `Missing recording start time, recording: ${JSON.stringify(recording)}`));
        } else if (!Number.isInteger(recording.start_time)) {
            return buildResponse(4, 400,
                warn("main", "event.body.meeting.recordings.recording.start_time", `recording start time should be an int, recording: ${JSON.stringify(recording)}`));
        }

        if (!recording.play_url) {
            return buildResponse(4, 400,
                warn("main", "event.body.meeting.recordings.recording.play_url", `Missing recording play url, recording: ${JSON.stringify(recording)}`));
        } else if (typeof recording.play_url !== "string") {
            return buildResponse(4, 400,
                warn("main", "event.body.meeting.recordings.recording.play_url", `recording play url should be a string, recording: ${JSON.stringify(recording)}`));
        }
    }
...
function formatInputForUpdateMeetingRecording(request, recording) {
    return {
        functionName: "UPDATE_MEETING_RECORDING",
        headers: {
            access_token: process.env.JWT,
            refresh_token: "dummy",
            user: useremail,
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        const insertMeetingRecordingsRequest = formatInputForInsertMeetingRecordings(parsedRequest.meeting);
        ...
        for (let recording of parsedRequest.meeting.recordings) {
            const updateMeetingRecording = formatInputForUpdateMeetingRecording(parsedRequest, recording);
            log("main", "updateMeetingRecording", `Request: ${JSON.stringify(updateMeetingRecording)}`);
            const updateMeetingRecordingResponse = await queryRDS(updateMeetingRecording);
            if (updateMeetingRecordingResponse.error !== 0) {
                return updateMeetingRecordingResponse;
            }
            log("main", "updateMeetingRecordingResponse", `Response: ${JSON.stringify(updateMeetingRecordingResponse)}`);
        }
        ...
        return buildResponse(1, 500,
            warn("main", "err",
                `Error in MeetingRecorded + ${JSON.stringify(err.stack) || "unknown error"}`));
    }
};
...
function getLogLine(funcname, process, log) {
    return `[[ MeetingRecorded - ${funcname} ]] -> ${process}, user:${useremail}, id: ${requestID}, logged -- ${log}`;
}